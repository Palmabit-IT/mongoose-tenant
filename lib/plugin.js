function multitenantPlugin(schema, options) {
  ...
  /*
   * throw err if there isn't tenant in conditions obj
   */
  function checkThereIsTenantInConditions(next) {
    if (!this._conditions || !this._conditions[tenant]) {
      return next(new Error('Tenant is required'))
    }

    next();
  }

  schema.pre('find', checkThereIsTenantInConditions);
